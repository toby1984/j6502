...
PRB 	Data Port B 	Monitoring/control of the 8 data lines of Port B. The lines are used for multiple purposes:
        ...
        Read: Bit 7: Timer B: Toggle/Impulse output (see register 15 bit 2)
            ...
            if ( offset == CIA_PRA )
            {
                final int pra = super.readByte( CIA_PRA , applySideEffects );
                return pra & joy2Mask;
            }
            if ( offset == CIA_PRB )
            {
            ...
            return super.readByte( offset , applySideEffects );
        }
    };
    ...
    public final CIA cia2 = new CIA("CIA #2" , AddressRange.range( 0xdd00, 0xde00 ) )
    {
        ...
        public void writeByte(int adr, byte value)
        {
            ...
            if (offset == CIA_PRA )
            {
                ...
                final int oldValue = super.readByteNoSideEffects(adr );
                ...
                if ( Constants.IEC_DEBUG_WIRE_LEVEL )
                {
            ...
            {
                if ( offset == CIA_DDRA && Constants.IEC_DEBUG_WIRE_LEVEL )
                {
        ...
        protected int readByte(int adr,boolean applySideEffects)
        {
            int value = super.readByte(adr,applySideEffects);
            final int offset = ( adr & 0xffff ) % 0x10; // registers are mirrored/repeated every 16 bytes
            if ( offset == 0 ) {
                /*
				 Bit 0..1: Select the position of the VIC-memory
...
// CIA #2: actually only 16 byte registers but mirrored in range $DD10-$DDFF
    ...
    public int readByteNoSideEffects(int adr)
    {
        ...
        final int offset = adr & 0xffff;
        if ( offset >= 0x800 && offset < 0x800+1024) {
            return colorRAMBank.readByteNoSideEffects( offset ); // intentionally NOT offset - 0x800 as colorRAMBank is actually the whole bank #5 (D000-DFFF) and not just the 1 KB block starting at $D800
        }
        if ( offset >= 0xc00 && offset <= 0xcff) { // CIA #1 $DC00-$DCFF
            return cia1.readByteNoSideEffects( offset - 0xc00 );
        }
        if ( offset >= 0xd00 && offset <= 0xdff ) { // CIA #2 $DD10-$DDFF
            return cia2.readByteNoSideEffects( offset - 0xd00 );
        }
        if ( offset < 0x002f ) { // $D000 - $D02F ... VIC
            return vic.readByteNoSideEffects( offset );
        }
