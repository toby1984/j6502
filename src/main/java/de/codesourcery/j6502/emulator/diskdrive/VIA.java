...
{
	private static final int IRQBIT_TIMER1_TIMEOUT = 1<<6;
	private static final int IRQBIT_TIMER2_TIMEOUT = 1<<5;
	private static final int IRQBIT_CB1_ACTIVE_EDGE = 1<<4;
	private static final int IRQBIT_CB2_ACTIVE_EDGE = 1<<3;
	private static final int IRQBIT_SHIFTS_COMPLETED = 1<<2;
	private static final int IRQBIT_CA1_ACTIVE_EDGE = 1<<1;
	private static final int IRQBIT_CA2_ACTIVE_EDGE = 1<<0;
	...
	{
		public void portAChanged(VIA via,VIA.Port port);
		public void portBChanged(VIA via,VIA.Port port);
		public void ca1Changed(VIA via,VIA.Port port);
		public void ca2Changed(VIA via,VIA.Port port);
		public void cb1Changed(VIA via,VIA.Port port);
		public void cb2Changed(VIA via,VIA.Port port);
	}
	...
	protected class Port
	{
		...
		public void tick()
		{
			...
			result |= value;
			externalPins = result;
			if ( oldValue != externalPins ) 
			{
				if ( portName == PortName.A ) {
					changeListener.portAChanged( VIA.this , this );
				} else {
					changeListener.portBChanged( VIA.this , this );
				}
			}
		}
		...
		public void setCA1(boolean ca1) {
			if ( this.ca1 != ca1 ) {
				this.ca1 = ca1;
				changeListener.ca1Changed( VIA.this , this );
			} else {
				this.ca1 = ca1;
			}
		}
		...
		public void setCA2(boolean ca2) {
			if ( this.ca2 != ca2 ) {
				this.ca2 = ca2;
				changeListener.ca2Changed( VIA.this , this );
			} else {
				this.ca2 = ca2;
			}			
		}
		...
		public void setCB1(boolean cb1) {
			if ( this.cb1 != cb1 ) {
				this.cb1 = cb1;
				changeListener.cb1Changed( VIA.this , this );
			} else {
				this.cb1 = cb1;
			}		
		}
		...
		public void setCB2(boolean cb2) {
			if ( this.cb2 != cb2 ) {
				this.cb2 = cb2;
				changeListener.cb2Changed( VIA.this , this );
			} else {
				this.cb2 = cb2;
			}			
		}
	...
	public int readByte(int offset)
	{
		switch( offset )
		{
			...
			case T1LH:
				return t1latchhi;
			case T2CL:
				irqFlags &= ~IRQBIT_TIMER2_TIMEOUT;
				return (timer2 & 0xff);
			case T2CH:
	...
	public void writeByte(int offset, byte value)
	{
		switch( offset )
		{
			...
			case T1CH:
				...
				timer1 |= (value & 0xff) << 8;
				irqFlags &= ~IRQBIT_TIMER1_TIMEOUT;
				break;
			case T1LL:
				t1latchlo = value & 0xff;
				break;
			case T1LH:
				irqFlags = irqFlags & ~(1<<6); // clear timer #1 irq
			...
			case PCR:
				...
				break;
			case IFR:
				this.irqFlags = this.irqFlags & ~(value & 0xff);
				break;
			case IER:
				this.irqEnable = value & 0xff;
				break;
			case PORTA_NOHANDSHAKE:
				portA.writeRegister( value );
				break;
	...
	public void tick()
	{
		if ( timer1Running && --timer1 == 0 ) {
			switch( timer1Mode ) 
			{
				case IRQ_ON_LOAD:
				case CONTINUOUS_IRQ:
					if ( (irqEnable & IRQBIT_TIMER1_TIMEOUT) != 0 ) {
		...
		{
			if ( (irqEnable & IRQBIT_TIMER2_TIMEOUT) != 0 ) {
