...
import de.codesourcery.j6502.assembler.parser.ast.InitializedMemoryNode.Type;
...
public class Parser
{
    private static final String[] META_COMMANDS = {"incbin" , "byte" , "word" , "equ" };
    ...
    private boolean parseLabel() {
        ...
        if ( lexer.peek( TokenType.DOT ) )
        {
            ...
            if ( ! lexer.eof() && isMetaInstruction( lexer.peek() ) ) 
            {
    ...
    private static boolean isMetaInstruction(Token tok) 
    {
        if ( tok.hasType(TokenType.CHARACTERS) ) 
        {
            for ( String expected : META_COMMANDS) 
            {
                if ( expected.equalsIgnoreCase(tok.text) ) {
                    return true;
                }
            }
        }
    ...
    private boolean parseMeta()
    {
        ...
        if ( lexer.peek( TokenType.DOT ) ) 
        {
            ...
            if ( lexer.peek().isCharacters("incbin") ) // .incbin "file"
            {
                lexer.next();
                final StringLiteralNode text = parseString();
                if ( text == null ) {
                    fail(".incbin requires a file name in double quotes"); // always throws ParseException
                    return false; // make compiler happy
                }
                final TextRegion region = new TextRegion( dot.offset , lexer.currentOffset() - dot.offset );
                currentNode.addChild( new IncludeBinaryNode( text.value, region ) );
                return true;
            }

            if ( lexer.peek().isCharacters("byte") || lexer.peek().isCharacters("word") ) // .byte / .word
            {
                final Token token = lexer.next();
                final Type width = token.isCharacters("byte") ? InitializedMemoryNode.Type.BYTES : InitializedMemoryNode.Type.WORDS; 
                final TextRegion region = new TextRegion( dot.offset , lexer.currentOffset() - dot.offset );
                final InitializedMemoryNode node = new InitializedMemoryNode( region , width );
                node.addChildren( parseMemInitList() );
                currentNode.addChild( node );
                return true;
            }

            if ( lexer.peek().isCharacters("equ") && labelOnCurrentLine != null ) // .equ
            {
                lexer.next();
                ...
                final EquNode node = new EquNode( labelOnCurrentLine.identifier , new TextRegion( labelOnCurrentLine.getTextRegion() ).merge( dot.region() ) );
    ...
    private StringLiteralNode parseString() 
    {
        ...
        try
        {
            ...
            return new StringLiteralNode( buffer.toString(), new TextRegion( startingQuotes.offset , buffer.length()+2 ) );            
        } finally {
            lexer.setSkipWhitespace( true );
        }            
    }
